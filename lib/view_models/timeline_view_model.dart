...
class TimelineViewModel extends ChangeNotifier {
  ...
  void _executeClipDelete(TimelineAction action, bool isUndo) {
    if (isUndo) {
      ...
      if (trackId == _dawViewModel.beatTrack.id) {
        ...
        // Bolt ⚡: Replace try-catch block and firstWhere with indexWhere to avoid expensive exception generation
        final trackIndex = _dawViewModel.vocalTracks.indexWhere((track) => track.id == trackId);
        if (trackIndex != -1) {
  ...
  void trimClip(String clipId, Duration newStartTime, Duration newEndTime) {
    ...
    for (final track in _dawViewModel.vocalTracks) {
      // Bolt ⚡: Replace firstWhere with indexWhere to prevent unnecessary exception throwing when looping
      final clipIndex = track.clips.indexWhere((c) => c.id == clipId);
      if (clipIndex == -1) continue;
  ...
  AudioClip? _findClipById(String clipId) {
    for (final track in _dawViewModel.vocalTracks) {
      ...
      if (idx != -1) return track.clips[idx];
    }

    final beatIdx = _dawViewModel.beatTrack.clips.indexWhere(
      (c) => c.id == clipId,
    );
    if (beatIdx != -1) return _dawViewModel.beatTrack.clips[beatIdx];

    if (_dawViewModel.mixedVocalTrack != null) {
      final mixedIdx = _dawViewModel.mixedVocalTrack!.clips.indexWhere(
        (c) => c.id == clipId,
      );
      if (mixedIdx != -1) return _dawViewModel.mixedVocalTrack!.clips[mixedIdx];
    }

    if (_dawViewModel.masteredSongTrack != null) {
      final masterIdx = _dawViewModel.masteredSongTrack!.clips.indexWhere(
        (c) => c.id == clipId,
      );
      if (masterIdx != -1)
        return _dawViewModel.masteredSongTrack!.clips[masterIdx];
    }
