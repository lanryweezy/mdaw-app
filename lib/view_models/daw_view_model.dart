...
class DawViewModel extends ChangeNotifier {
  ...
  AudioEffect? getEffect(String effectName) {
    for (final e in _effects.values) {
      if (e.name.toLowerCase() == effectName.toLowerCase()) {
        return e;
      }
    }
    return null;
  ...
  Future<void> applyMastering(dynamic preset) async {
    ...
    try {
      ...
      final index = vocalTracks.indexWhere((t) => t.hasAudio);
      if (index == -1) return;

      final vocalPath = vocalTracks[index].clips.first.path;
