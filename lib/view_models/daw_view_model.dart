...
class DawViewModel extends ChangeNotifier {
  ...
  void toggleEffect(String effectName, bool isEnabled) {
    ...
    final effect = _effects[id] ??= AudioEffect(
      name: effectName,
      id: id,
      isEnabled: isEnabled,
    );
  ...
  void openEffectSettings(String effectName) {
    ...
    if (effect == null) throw Exception('Effect not found: $effectName');
  ...
  Future<void> applyMastering(dynamic preset) async {
    ...
    try {
      final vocalTrackIndex = vocalTracks.indexWhere((t) => t.hasAudio);
      if (vocalTrackIndex == -1 || beatTrack.clips.isEmpty) return;
      final vocalPath = vocalTracks[vocalTrackIndex].clips.first.path;
  ...
  Future<void> separateStems() async {
    ...
    try {
      ...
      if (vocalsPath != null) {
        final trackIndex = vocalTracks.indexWhere((t) => !t.hasAudio);
        final targetVocalTrack = trackIndex != -1
            ? vocalTracks[trackIndex]
            : vocalTracks.last;
        final clipId =
