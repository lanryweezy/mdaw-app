...
class Track {
  ...
  AudioClip? getClip(String clipId) {
    // ⚡ Bolt: Replaced firstWhere (which throws StateError if not found) with indexWhere
    // This avoids expensive exception-based control flow that causes jank
  ...
  bool operator ==(Object other) {
    ...
    // Check clips collection
    if (other.clips.length != clips.length) return false;
    for (int i = 0; i < clips.length; i++) {
      if (other.clips[i] != clips[i]) return false;
    }
